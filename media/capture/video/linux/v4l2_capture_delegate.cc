...
void V4L2CaptureDelegate::SetPhotoOptions(
    ...
    settings->focus_mode == mojom::MeteringMode::CONTINUOUS))
  {
    ...
    auto_focus.value = settings->focus_mode == mojom::MeteringMode::MANUAL ? false : true;
    if (DoIoctl(VIDIOC_S_CTRL, &auto_focus) < 0)
      DPLOG(ERROR) << "setting focusMode to " << (settings->focus_mode == mojom::MeteringMode::MANUAL ? "manual" : "continuous");
  }
